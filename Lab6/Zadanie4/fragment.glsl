...
vec3 calculateLight(Light light, Material material){
	...
	if(lightQuality){
		...
		if(!lightMode){
			lightValue = max(lightValue, calculatePhongLight(position.xyz, fragNormal, light, material));
		} else {
			lightValue = max(lightValue, calculateBlinnPhongLight(position.xyz, fragNormal, light, material));
		}
	} else {
...
vec3 calculateDirectionalLight(Light light, Material material){
	vec3 lightValue = vec3(0.0);
	if(lightQuality){
		vec4 position = matModel * realCameraPos;
		if(!lightMode){
			lightValue = max(lightValue, calculateDirectionalPhongLight(position.xyz, fragNormal, light, material));
		} else {
			lightValue = max(lightValue, calculateDirectionalBlinnPhongLight(position.xyz, fragNormal, light, material));
		}
	} else {
		lightValue = lightCoef;
	}
...
void main()
{
	...
	if(lightVisibility){
		vec3 lightCoef = calculateDirectionalLight(dirLight[0], material);
		lightCoef = max(lightCoef, calculateLight(light[0], material));
