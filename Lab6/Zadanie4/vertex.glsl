...
vec3 calculateDirectionalLight(Light light, Material material){
	...
	if(!lightQuality){
		if(!lightMode){
			lightValue = max(lightValue, calculateDirectionalPhongLight(matModel * inPosition, fragNormal, light, material));
		} else {
			lightValue = max(lightValue, calculateDirectionalBlinnPhongLight(matModel * inPosition, fragNormal, light, material));
		}
	} else {
		lightValue = lightCoef;
	}
...
void main()
{
	...
	if(lightVisibility){
		lightCoef = calculateDirectionalLight(dirLight[0], material);
