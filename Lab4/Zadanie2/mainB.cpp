...
void DisplayScene()
{
	...
	for (Model model : models){
		...
		if (modelIndex > 0 && modelIndex < 3) {
			models[modelIndex].matrix = glm::translate(models[modelIndex].matrix, glm::vec3(0, -3, 0));
			models[modelIndex].matrix = glm::rotate(models[modelIndex].matrix, one_degree, glm::vec3(1, 0, 0));
			models[modelIndex].matrix = glm::translate(models[modelIndex].matrix, glm::vec3(0, 3, 0));
		}
		else if (modelIndex == 3) {
			models[modelIndex].matrix = glm::scale(models[modelIndex].matrix, glm::vec3(0.25, 0.25, 2));
			models[modelIndex].matrix = glm::rotate(models[modelIndex].matrix, one_degree, glm::vec3(1, 0, 0));
			models[modelIndex].matrix = glm::scale(models[modelIndex].matrix, glm::vec3(4, 4, 0.5));
		}
		modelIndex++;
	}
	// Wylaczanie
...
void Initialize()
{
	...
	cube.load("src/cube.obj");
	cube.matrix = glm::translate(cube.matrix, glm::vec3(0, 8, 0));
	cube.matrix = glm::scale(cube.matrix, glm::vec3(0.5, 8, 0.5));
	models.push_back(cube);

	Model cone1;
	cone1.program->id = glCreateProgram();
	cone1.matrix = glm::rotate(cone1.matrix, 90 * one_degree, glm::vec3(0, 0, 1));
	cone1.matrix = glm::translate(cone1.matrix, glm::vec3(0, 3, 0));
	...
	cone2.matrix = glm::rotate(cone2.matrix, 180 * one_degree, glm::vec3(1, 0, 0));
	cone2.matrix = glm::translate(cone2.matrix, glm::vec3(0, 3, 0));
	cone2.load("src/cone.obj");
	models.push_back(cone2);

	Model torus;
	torus.program->id = glCreateProgram();
	torus.load("src/torus.obj");
	torus.matrix = glm::scale(torus.matrix, glm::vec3(4, 0.5, 0.5));
	torus.matrix = glm::rotate(torus.matrix, 90 * one_degree, glm::vec3(0, 0, 1));
...
int main(int argc, char* argv[])
{
	...
	glutCreateWindow("Szablon programu w OpenGL - Lab 4 - Zadanie 2 B");
