...
vec3 calculateLight(Light light[32], Material material, vec4 displacedFragPos){
	...
	if(!lightQuality){
		for (int i = 0; i < 32; ++i) {
		    if(!lightMode){
			    lightValue = max(lightValue, calculatePhongLight(matModel * displacedFragPos, fragNormal, light[i], material));
		    } else {
			    lightValue = max(lightValue, calculateBlinnPhongLight(matModel * displacedFragPos, fragNormal, light[i], material));
		    }
	    } 
	}else {
...
vec3 calculateDirectionalLight(Light light[32], Material material, vec4 displacedFragPos){
	...
	if(!lightQuality){
		for (int i = 0; i < 32; ++i) {
		    if(!lightMode){
			    lightValue = max(lightValue, calculateDirectionalPhongLight(matModel * displacedFragPos, fragNormal, light[i], material));
		    } else {
			    lightValue = max(lightValue, calculateDirectionalBlinnPhongLight(matModel * displacedFragPos, fragNormal, light[i], material));
		    }
	    }
    } else {
		lightValue = lightCoef;
...
void main()
{
    vec2 normalizedMapPos = (worldPosition + 64.0) / 128.0;
	float terrainHeight = (texture(heightmap, vec2(normalizedMapPos.x,1.0-normalizedMapPos.y)).r - 0.4) * (25/scale.y);
