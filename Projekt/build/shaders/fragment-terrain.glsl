...
void main()
{
	...
	if(useTexture){
		vec4 texColor1 = texture(tex1, fragUV*512);
		vec4 texColor2 = texture(tex2, fragUV*512);
		vec4 texColor3 = texture(tex3, fragUV*512);
		...
		float dhx = heightRight - heightLeft;
		float dhy = heightDown - heightUp;
		float slope = atan(sqrt(dhx * dhx + dhy * dhy));

		baseColor = vec4(mix(texColor1.rgb, texColor2.rgb, clamp(slope*24,0.0,1.0)), 1.0);
		baseColor = vec4(mix(texColor3.rgb, baseColor.rgb, clamp(fragHeight,0.0,1.0)), 1.0);
		//baseColor = vec4(vec3(fragHeight),1.0);
		//if(fragHeight+1<0.0) {
		//	baseColor= vec4(0.0, 0.0, 1.0, 1.0);
		//}
		float minValue = -10.0;
		float maxValue = 1.0;
		float t = clamp((fragHeight - minValue) / (maxValue - minValue), 0.0, 1.0);
		baseColor = mix(vec4(0.094, 0.541, 0.553, 1.0), baseColor, smoothstep(0.0, 1.0, t));
	}else{
	...
	if(lightVisibility){
		...
		float minValue = -10.0;
		float maxValue = 1.0;
		float t = clamp(((fragHeight+1) - minValue) / (maxValue - minValue), 0.0, 1.0);
		finalColor.rgb *= (lightCoef * (1 - (shadowPart*t*t*t)));
	}
	if(fogVisibility){
