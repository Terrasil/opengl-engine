...
GLuint idVBO_pSizes; // bufora na rozmiary punktow
...
void DisplayScene()
{
	...
	glDrawArrays(GL_POINTS, 0, TRIANGLES_VECTOR->size() * 3);
	glBindVertexArray(0);
...
// ---------------------------------------
void Initialize(std::vector<Triangle> &triangles, std::vector<Color> &colors, std::vector<PointSize>& pSizes)
{
	// -------------------------------------------------
	...
	glGenBuffers(1, &idVBO_pSizes);
	glBindBuffer(GL_ARRAY_BUFFER, idVBO_pSizes);
	glBufferData(GL_ARRAY_BUFFER, sizeof(PointSize) * pSizes.size(), &(pSizes[0]), GL_STATIC_DRAW);
	...
	glAttachShader(idProgram, LoadShader(GL_VERTEX_SHADER, "vertexB.glsl"));
...
void initializePointSizes(std::vector<PointSize>& pSizes, int N) {
	srand(time(NULL));
	for (int i = 0; i < N; i++) {
		pSizes.push_back(PointSize((float) (rand() % 20)));
	}
}
...
int main(int argc, char* argv[])
{
	...
	std::vector<PointSize> pSizes;
	initializePointSizes(pSizes, N);
	...
	// Inicjalizacja
	Initialize(triangles, colors, pSizes);
	...
	glDeleteVertexArrays(1, &idVBO_pSizes);
