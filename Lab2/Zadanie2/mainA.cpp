...
void Initialize(std::vector<Triangle> &triangles, std::vector<Color> &colors)
{
	...
	glAttachShader(idProgram1, LoadShader(GL_FRAGMENT_SHADER, "fragment.glsl"));
	...
	glAttachShader(idProgram2, LoadShader(GL_FRAGMENT_SHADER, "fragmentGrayscale.glsl"));
...
void Keyboard(unsigned char key, int x, int y)
{
	...
	case ' ':	// Space key
		...
		programID = !programID;
		glUseProgram(programID ? idProgram1 : idProgram2);
		glUniform1i(glGetUniformLocation(programID, "programID"), programID);
		glutPostRedisplay();
		break;
	}
}
