...
void DisplayScene()
{
	...
	std::vector<Model> modelsWithGLRGBA;
	std::vector<Model> modelsWithoutGLRGBA;
	for (Model model : models) {
		if (model.getMesh()->getTexture()->getFormat() == GL_RGBA) {
			modelsWithGLRGBA.push_back(model);
		}
		else {
			modelsWithoutGLRGBA.push_back(model);
		}
	}
	...
	for (Model model : modelsWithoutGLRGBA) {
		model.draw(cameraPos, matProj, matView);
	}
	for (Model model : modelsWithGLRGBA) {
		model.draw(cameraPos, matProj, matView); 
	}
